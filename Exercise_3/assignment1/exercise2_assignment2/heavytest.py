...
def delete_files_if_exist(filenames):
...
def run_test_program(num_runs, max_threads, thread_step, max_ints, max_value):
    # Delete the .std files if they exist
    delete_files_if_exist(["stdout.std", "stderr.std"])
    ...
    while(num_threads < max_threads):
        ...
        for _ in range(num_runs):
            ...
            # Dictionary to track how many times each number is calculated
            number_counts = defaultdict(int)

            # Create the command with the random values
            command = ['./PrimalityTester.bin', str(num_threads)] + list(map(str, integers))
            ...
            for line in result.stdout.splitlines():
                ...
                if match:
                    ...
                    number_counts[num] += 1  # Increment calculation count for the number
            ...
            # Check if any number was calculated more times than it was given
            for num, count in number_counts.items():
                input_count = integers.count(num)
                if count > input_count:
                    print(f"Error: Number {num} was calculated {count} times but appeared only {input_count} times in the input.")
